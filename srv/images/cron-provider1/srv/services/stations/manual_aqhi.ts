...
import { _read_data, _write_data, formatToLocalISO, parseNumber, normalizeTimestamp, type Timestamp } from "../../utils";
import { StationRecord, type PollutantKey } from "../../types/contracts";
...
type ReadingsByTimestampProps = {
    readingsByTimestamp: Record<string, StationRecord>;
...
function apply_aqi_formula({
    ...
    pm25,
    h2s
}): [PollutantKey, number][] | null {
    
    const isValid = (num: any) => parseNumber(num) !== null;
    const pollutant_types: PollutantKey[] = [
        "no2",
        "so2",
        "co",
        "o3",
    ...
    const AQIs = Object.fromEntries(
        pollutant_types.map(p => [p, null])
    ...
    // all AQIs are null
    if (!entries.some(([p, val]) => val !== null)) return null;

    const sortedAQIs = Object.entries(AQIs)
        .sort(([, valueA], [, valueB]) => valueB - valueA);

    return sortedAQIs as [PollutantKey, number | null][];
 ...
 */
const aqhiCanBeCalculated = (
    reference_timestamp: number,
    sorted_timestamps: string[],       // most to least recent
    station_records: Record<string, StationRecord>
): {
    ...
    const aqhi_3_pollutants_missing = (
...
const extract_AQHI_pollutant_readings = (
    records: StationRecord[]       // each index corresponds to a timestamp
): AQHIPollutants[] => {
...
function calculate_station_AQHI({
    ...
    aqhi: number | null,
    reasonForMissingAQHI: string | undefined
} {
    const reference_timestamp = normalizeTimestamp(asOf);
    ...
    const {
        canCalculate: continueCalculation,
        reasonIfFalse: reasonForMissingAQHI       // show this for null AQHIs
    ...
    const relevantReadings = recentTimestamps
        ...
        .filter(reading => reading !== undefined && reading !== null) as StationRecord[];
    ...
    // apply formula
    let                         aqhi = null
    if (continueCalculation)    aqhi = apply_AQHI_formula(pollutantAverages);
...
function use_AQI_instead({
    ...
    if (aqi > aqhi) {
        if (determining_pollutant === "pm25")   return true;
...
export function execute_AQHI_calculation_flow({
    ...
    aqhi: number | null;
    aqis_ranked: ReturnType<typeof calculate_station_AQI> | null;
    ...
    if (reasonForMissingAQHI !== null) extraInfo['reasonForMissingAQHI'] = reasonForMissingAQHI
...
function mergeStationData(
    mainData: Record<string, StationRecord>, 
    baseFolderPath: string,
    skipFirstTwo: boolean=true  // set false if you've already excluded the first two
) {
    ...
    for (const [i, timestamp] of timestamps_asc.entries()) {
        ...
        const targetFile = path.join(baseFolderPath, folderName, 'overall_stats.json');
        ...
        const aqhi_stats_file = path.join(baseFolderPath, folderName, 'aqhi_calculations.json');
        ...
        if (fs.existsSync(aqhi_stats_file)) {
            try {
                ...
                if ("manual" in mergedRecord) {
                    mergedRecord.manual = {
                        ...(mergedRecord["manual"] as Record<string, any>),
                        ...toKeep
                    }
                }
...
(async () => {
    ...
    const mismatches =_read_data(`./outputs/comparisons/${stationKey}/aqhi_mismatch.json`);
    ...
    _write_data(`./outputs/comparisons/${stationKey}/null_aqhi.json`, filtered_mismatches);
